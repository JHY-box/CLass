...
public String save(HttpServletResponse response,
    ...
    /** 2) 강제 페이지 이동. */
    // 이 페이지에 머물렀다는 사실이 웹 브라우저의 history에 남자 않는다.
...
@PostMapping("/cookie/popup_close")
    ...
    /* 1) 쿠키 저장하기 */
    // 60초 간 유요한 쿠키 설정
    // 실제 상용화 시에는 domain을 설정해야한다.
    WebHelper.getInstance().whiteCookie(response, "no-opne", noOpen, 60);
