...
import kr.yjh.fileupload.interceptors.MyInterceptor;
...
public class MyWebConfig implements WebMvcConfigurer{
    ...
    // 이 과정에서 MyInterceptor안에 @Autowired로 선언된 WrbHelper 객체도 자동 주입된다.
    @Autowired
    private MyInterceptor myInterceptor;



    /** 업로드 된 ㅏ일이 저장될 경호 (application.properties로부터 읽어옴) */          // 06.17 FileUpload
    // -->
    @Value("${upload.dir}")
    private String uploadDir;

    /** 업로드 된 파일이 노출될 URL 경로  (application.properties로부터 읽어옴)*/      // 06.17 FileUpload
    @Value("${upload.url}")
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        // 업로드 된 파일이 저장될 경로와 URL상의 경로를 맵핑
