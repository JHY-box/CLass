...
/**
 * 특정 테이블(Departments)에 대한 CRUD기능을 정의하는 인터페이스
 * 
 * C - CREATE(생성) --> INSERT문 --> 글쓰기
 * R - READ(조회) --> SELECT문 --> 목록검색, 상세보기
 * U - UPDATE(수정) --> UPDATE문 --> 글 수정
 * D - DELETE(삭제) --> DELETE문 --> 글 삭제
 * 
 * 모든 메서드의 파라미터는 테이블 구조를 정의하는 Beans 클래스 타입
 * 
 * INSERT, UPDATE, DELETE의 경우 int형을 리턴 : 적용된 데이터 수
 * 
 * SELECT - 단일행 조회 - Beans 클래스 타입을 리턴
 * SELECT - 다중행 조회 - List<Beans 클래스> 타입을 리턴
 * 
 * 메서드 이름은 자유롭게 설정
 */
@Mapper
public interface DepartmentMapper {
     ...
     */
    @Select("<script>"
            + "SELECT id, dname, loc, phone, email, established, homepage FROM departments"
            // Dynamic SQL
