...
import com.jamesobin.database.helpers.FileHelper;
...
public class ProfessorController {
    ...
    @ResponseBody
    @PutMapping("/professor/edit_ok/{id}")
    public void editOk (
        Model model
        ...
        , @RequestParam(value="sal", required=true) String sal
        , @RequestParam(value="hiredate", required=true) String hiredate
        , @RequestParam(value="comm", required=true) String comm
        , @RequestParam(value="email", required=true) String email
        , @RequestParam(value="phone", required=true) String phone
        , @RequestParam(value="photo_url", required=true) MultipartFile photoUrl
        ...
        if(existingPhotoUrl != null && !existingPhotoUrl.isEmpty()) {
            ...
            if("Y".equals(deletePhoto)) {
                try {
                    ...
                    log.debug("기존 사진이 삭제되었습니다.");
                }
            } else if (uploadItem == null) {
                // 사진을 새로 업로드하지 않은 경우 기존 사진 경로를 유지
