...
// arr.lastIndexOf(item, from)는 위 메서드와 동인한  기능을 하는데, 검색을 끝에서부터 시작한다.
...
// arr.includes(item, from)는 인덱스 from부터  시작해 itme이 있는지를 검색하는데, 해달하는 요소를 발견하면 true를 반환한다.
...
// includes 는 NaN도 제대로 처리한다는 점에서 indexOf/lastIndexOf와 약간의 차이가 있다.
const arr2 = [NaN];
