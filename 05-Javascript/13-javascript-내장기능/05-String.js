 ...
 * 문자열은 그 자체가 객체.
 * 
 * 객체라는것은 그 안에 멤버변수(프로퍼티)와 메서드(함수)가 내장되어 있음을 의미.
 * 
 * 그러므로 일반적으로 생성하는 문자열 변수 안에는 메서드와 프로터티가 자동으로 내장된다.
 * 
...
// 파라미터로 전당된 내용이 처음 나타나는 위치를 리턴한다.
...
console.log("`i`가 첫 번째로 나타나는 위 : " + p2);
console.log("`i`가 두 번째로 나타나는 위 : " + p3);
...
// 첫번째 파라미터와 일치하는 내용이 둘 이상 존재할 경우 첫 번째 항목만 변경한다.
